...
private:
  ...
  void swapThenElse(clang::IfStmt *stmt) {
    std::string thenBranch = getSourceText(stmt->getThen()).str();
    std::string elseBranch = stmt->getElse() ? getSourceText(stmt->getElse()).str() : "";
  ...
  void invertCondition(clang::IfStmt *stmt) {
    std::string condition = getSourceText(stmt->getCond()).str();
...
static RegisterMutator<RandomizeIfElseBranches> M(
    "RandomizeIfElseBranches", 
