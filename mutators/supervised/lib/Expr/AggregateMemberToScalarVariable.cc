...
static RegisterMutator<AggregateMemberToScalarVariable>
    M("aggregate-member-to-scalar-variable",
...
bool AggregateMemberToScalarVariable::mutate() {
  ...
  std::string baseName = getSourceText(arrayExpr->getBase()).str();
  std::string indexName = getSourceText(arrayExpr->getIdx()).str();
