...
void MutatorManager::printMutators() {
  json j = json::object();
  j["suggested"] = json::array();
  j["not-suggested"] = json::array();
  for (auto &kvpair : mutatorsMap)
    if (kvpair.first == "remove-unused-function" ||
        kvpair.first == "remove-unused-variable" ||
        kvpair.first == "mutate-suffix-unaryop" ||
        kvpair.first == "remove-suffix-unaryop")
      j["not-suggested"].push_back(kvpair.first);
    else
      j["suggested"].push_back(kvpair.first);
  j["not-suggested-reason"] = json::array({"will clean source code too fast",
      "suffix post-exprs are often loop incr, change it usually result in "
      "infinite loop"});
  llvm::outs() << j.dump(2) << "\n";
}
...
bool MutatorManager::setSourceText(const std::string &sourceText,
  ...
  llvm::Triple T(TargetOpts.Triple);
  CompilerInvocation &Invocation = CI->getInvocation();
  InputKind IK = Language::C;

  Invocation.setLangDefaults(CI->getLangOpts(), IK.getLanguage(), T, Includes);
  ...
  // Create a memory buffer from the string
  bufferOwner = llvm::MemoryBuffer::getMemBufferCopy(sourceText, fileName);
  llvm::MemoryBufferRef BufferRef = bufferOwner->getMemBufferRef();
  
  // Initialize the source manager with the memory buffer
  if (!CI->InitializeSourceManager(FrontendInputFile(BufferRef, IK))) {
    return false;
  }
