...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  fsrv->total_execs++;
  fsrv->total_compiles += !WIFSTOPPED(fsrv->child_status)
    && !fsrv->last_run_timed_out && WEXITSTATUS(fsrv->child_status) == 0;

  /* Persistent records */
  if (fsrv->afl_ptr && (u64)time(NULL) > fsrv->last_record_time + 60 * 10) {
    fsrv->last_record_time = time(NULL);

    char *records_fn = alloc_printf("%s/records", ((afl_state_t *)fsrv->afl_ptr)->out_dir);
    FILE *fp = fopen(records_fn, "w+");
    if (!unlikely(fp)) { PFATAL("Unable to create '%s'", records_fn); }
    fprintf(fp, "%llu,%llu\n", fsrv->total_execs, fsrv->total_compiles);
    fclose(fp);
  }
